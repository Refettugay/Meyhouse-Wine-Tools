...
const COLUMN_TINTS = [
  ...
  { bg: "bg-amber-50/50", head: "bg-amber-100/70" },
...
export function BeverageMatrix({ tab, initial, locations, selectedLocationId }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Location selector */}
      <div className="bg-white border border-stone-200 rounded-xl p-3 flex items-center justify-between flex-wrap gap-2">
        <div className="text-sm flex items-center gap-2">
          <span className="text-stone-600 font-medium">Location:</span>
          <select
            value={selectedLocationId}
            onChange={(e) => handleLocationChange(e.target.value)}
            className="px-3 py-1.5 border border-stone-300 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:border-amber-500 bg-white"
          >
      ...
      <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3 flex items-center justify-between flex-wrap gap-2">
        <div className="text-sm">
          <span className="text-stone-600">Tab default target cost %:</span>
          {editingTabDefault ? (
            <input
              type="number"
              step="0.5"
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 text-xs">
        ...
        <StatChip label="Near" value={stats.near} color="text-amber-700 bg-amber-50" />
        ...
        <StatChip label="No Price" value={stats.noPrice} color="text-stone-600 bg-stone-100" />
        <StatChip label="No Cost" value={stats.noCost} color="text-stone-600 bg-stone-100" />
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {data.orphans.length > 0 && (
          <button
            ...
            disabled={isPending}
            className="inline-flex items-center gap-2 px-3 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-lg text-sm font-medium disabled:opacity-50"
          >
      ...
      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="text-left text-xs font-semibold text-stone-600 uppercase tracking-wide border-b border-stone-200">
                <th
                  className="px-3 py-2 bg-stone-50 whitespace-nowrap cursor-pointer select-none hover:bg-stone-100"
                ...
                <th className="px-3 py-2 bg-stone-50 text-right whitespace-nowrap">Btl Size</th>
                <th className="px-3 py-2 bg-stone-50 text-right whitespace-nowrap">Btl Cost $</th>
                {data.pourColumns.map((col, idx) => {
                  const tint = COLUMN_TINTS[idx % COLUMN_TINTS.length];
                  return (
                    <th key={col.label} colSpan={2} className={`px-2 py-2 text-center whitespace-nowrap border-l border-stone-200 ${tint.head}`}>
                      <div className="font-bold text-stone-800 text-sm normal-case">{col.label}</div>
                      <div className="flex items-center justify-center gap-1 mt-0.5">
                        <span className="text-[10px] font-normal text-stone-500">target:</span>
                        {editingColumnTarget === col.label ? (
                          <input
                            ...
                            className="w-14 px-1 py-0 border border-amber-400 rounded text-xs"
                          />
                        ) : (
                          <button
                            onClick={() => {
                            ...
                            className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded hover:bg-white/60 text-xs font-bold ${
                              col.isFromTabConfig ? "text-stone-800" : "italic text-stone-500"
                            }`}
                ...
                <th className="px-3 py-2 bg-stone-50 whitespace-nowrap">Actions</th>
              </tr>
              <tr className="text-[10px] font-semibold text-stone-500 uppercase tracking-wide border-b border-stone-200">
                <th className="px-3 py-1 bg-stone-50"></th>
                <th className="px-3 py-1 bg-stone-50"></th>
                <th className="px-3 py-1 bg-stone-50"></th>
                <th className="px-3 py-1 bg-stone-50"></th>
                <th className="px-3 py-1 bg-stone-50"></th>
                {data.pourColumns.flatMap((col, idx) => {
                  const tint = COLUMN_TINTS[idx % COLUMN_TINTS.length];
                  return [
                    <th key={`${col.label}-cost-sub`} className={`px-2 py-1 text-right border-l border-stone-200 ${tint.head}`}>Cost %</th>,
            ...
            <tbody className="divide-y divide-stone-100">
              {filteredProducts.length === 0 && (
                <tr>
                  <td colSpan={5 + data.pourColumns.length * 2 + 1} className="px-3 py-12 text-center text-stone-400 italic">
              ...
              {filteredProducts.map((row) => (
                <tr key={row.ingredientId} className="hover:bg-stone-50/70">
                  ...
                  <td className="px-3 py-2 text-stone-900 font-medium">
                    {editingName === row.ingredientId ? (
                      <input
                        ...
                        className="w-full px-2 py-0.5 border border-amber-400 rounded text-sm"
                      />
                    ) : (
                      <button
                        onClick={() => {
                        ...
                        className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-amber-50 text-left"
                      >
                        {row.name}
                  ...
                  {/* Btl Size */}
                  <td className="px-3 py-2 text-right text-stone-700 font-mono tabular-nums whitespace-nowrap">
                    {editingBottleSize === row.ingredientId ? (
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-amber-50 font-mono tabular-nums"
                      >
                  ...
                  {data.pourColumns.flatMap((col, idx) => {
                    ...
                    if (!cell) {
                      return [
                        <td key={`${row.ingredientId}-${col.label}-cost-empty`} className={`px-2 py-2 text-center border-l border-stone-100 ${tint.bg} text-stone-300`}>
                          —
                        </td>,
                        <td key={`${row.ingredientId}-${col.label}-price-empty`} className={`px-2 py-2 text-center ${tint.bg}`}>
                          <button
                            ...
                            disabled={isPending}
                            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-amber-100 text-xs text-stone-500 hover:text-amber-700 disabled:opacity-50"
                    ...
                    return [
                        <td
                          ...
                          className={`px-2 py-2 text-right border-l border-stone-100 ${tint.bg} ${statusColorClass(cell.status)} whitespace-nowrap`}
                        >
                          <div className="flex flex-col items-end gap-0.5">
                            ...
                            {editingCellTarget === cell.priceId ? (
                              <input
                                ...
                                className="w-14 px-1 py-0 border border-amber-400 rounded text-[10px]"
                                placeholder="target"
                              />
                            ) : (
                              <button
                                ...
                                className={`inline-flex items-center gap-0.5 text-[10px] ${
                                  cell.rowTargetPct !== null ? "font-bold text-stone-700" : "italic text-stone-400"
                                }`}
                          ...
                          <div className="flex items-center justify-end gap-1">
                            {editingPrice === cell.priceId ? (
                              <input
                                ...
                                className="w-20 px-1 py-0.5 border border-amber-400 rounded text-right text-xs font-mono"
                              />
                              ...
                              <button
                                ...
                                disabled={isPending}
                                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-amber-100 text-amber-700 font-mono tabular-nums italic"
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-white/60 font-mono tabular-nums text-stone-400"
                              >
                                —
                                <Pencil className="w-3 h-3 opacity-40" />
                              </button>
                  ...
                  <td className="px-3 py-2 whitespace-nowrap">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-stone-100 hover:bg-amber-100 text-stone-700 text-xs"
          ...
          <div className="flex flex-wrap gap-2 items-end">
            <div>
              ...
              <input
                ...
                className="px-3 py-1.5 border border-stone-300 rounded-lg text-sm w-36"
              />
            </div>
            <div>
      ...
      {data.orphans.length > 0 && (
        <div className="bg-white border border-dashed border-stone-300 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-stone-200 bg-stone-50/70 flex items-center gap-2">
            ...
            <span className="text-xs text-stone-500">· Click any row to add default pours</span>
          </div>
              ...
              <tbody className="divide-y divide-stone-100">
                {data.orphans.map((o) => (
                  <tr key={o.ingredientId} className="hover:bg-amber-50/40">
                    <td className="px-3 py-1.5 text-stone-900 font-medium">{o.name}</td>
                    <td className="px-3 py-1.5 text-stone-500">{o.category ?? "—"}</td>
                    <td className="px-3 py-1.5 text-stone-500">{o.vendorName ?? "—"}</td>
                    <td className="px-3 py-1.5 text-right text-stone-700 font-mono tabular-nums">{formatCents(o.bottleCostCents)}</td>
                    <td className="px-3 py-1.5 text-right text-stone-700 font-mono tabular-nums whitespace-nowrap">{o.bottleSizeMl ? `${o.bottleSizeMl}ml` : "—"}</td>
