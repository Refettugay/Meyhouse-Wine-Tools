...
export function RecipeEditForm({
  ...
  const [portionCount, setPortionCount] = useState(recipe.portionCount);
  const [dilutionPct, setDilutionPct] = useState(recipe.dilutionPct);
  const [glassType, setGlassType] = useState(recipe.glassType || "");
  const [iceType, setIceType] = useState(recipe.iceType || "");
  const [garnish, setGarnish] = useState(recipe.garnish || "");
  const [pourFromBatch, setPourFromBatch] = useState(recipe.pourFromBatch || "");
  const [howTo, setHowTo] = useState(recipe.howTo || "");
  const [storageType, setStorageType] = useState(recipe.storageType);
  const [noBatching, setNoBatching] = useState(recipe.noBatching);
  const [costTargetPct, setCostTargetPct] = useState(
    recipe.costTargetPct?.toString() || ""
  );
  const [menuPrice, setMenuPrice] = useState(
    recipe.menuPrice?.toString() || ""
  );
  ...
  function addRow() {
    setRows([
      ...rows,
      { ingredientId: "", amount: 0, unit: "OZ", isTopOff: false, notes: "" },
    ]);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await updateRecipe(recipe.id, {
      ...
      menuPrice: menuPrice ? parseFloat(menuPrice) : undefined,
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={portionCount}
                  onChange={(e) =>
                    setPortionCount(parseInt(e.target.value) || 15)
                  }
                  min={1}
                  className="w-full px-3 py-2 bg-[var(--brand-cream)] border border-[var(--line)] rounded-lg text-[var(--brand-brown)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-olive)]"
                />
        ...
        <div className="bg-white border border-[var(--line)] rounded-xl p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-[var(--brand-brown)] mb-1">
                Cost Target % (leave empty for category default)
              </label>
              <input
                type="number"
                value={costTargetPct}
