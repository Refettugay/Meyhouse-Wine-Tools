    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white border border-[var(--line)] rounded-xl p-4">
          <p className="text-sm text-[var(--ink-muted)]">Total Recipes</p>
          <p className="text-2xl font-bold">{filtered.length}</p>
        </div>
        <div className="bg-white border border-[var(--line)] rounded-xl p-4">
          <p className="text-sm text-[var(--ink-muted)]">With Cost Data</p>
          <p className="text-2xl font-bold">{pricedCount}</p>
        </div>
        <div className="bg-white border border-[var(--line)] rounded-xl p-4">
          <p className="text-sm text-[var(--ink-muted)]">Avg Cost / Drink</p>
          <p className="text-2xl font-bold text-[var(--brand-olive)]">
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap transition-colors ${
            selectedCategory === "ALL"
              ...
              : "bg-[var(--brand-cream)] text-[var(--ink-muted)]"
          }`}
        >
          All
        </button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--line)]">
                <th className="text-left px-4 py-3 text-[var(--ink-muted)] font-medium">
                  Cocktail
                </th>
                <th className="text-right px-4 py-3 text-[var(--ink-muted)] font-medium">
                  Cost
                </th>
                <th className="text-right px-4 py-3 text-[var(--ink-muted)] font-medium">
                  Target %
            ...
            <tbody className="divide-y divide-[var(--line)]">
              {recipesWithCosts.map((recipe) => {
                ...
                return (
                  <tr key={recipe.id} className="hover:bg-[var(--brand-cream)]">
                    ...
                    <td className="px-4 py-3 text-right text-[var(--brand-brown)]">
                      {recipe.targetPct}%
                    </td>
                    <td className="px-4 py-3 text-right">
                      {recipe.suggested > 0 ? (
